<?php
//The article heat 文章热度榜
function most_comm_posts($days=30,$nums=5){
    global $wpdb;//WordPress 内置的访问数据库的类对象。该函数通过使用 global 关键字来将这个对象引入到本函数作用域中。
    ...
    $output = '';
    if(empty($result)){
        $output = '<li>'.__('偷懒ing...文章没写。','moedog').'</li>';
    }else{
        ...
        <div class="textwidget">
            <div class="widget-admin text-center">
                <p>
                    ...
                    <a href="<?php echo admin_url('/'); ?>"><i class="fa fa-dashboard"></i> <?php _e('个人中心','moedog'); ?> </a>
                    <a href="<?php echo wp_logout_url($redirect); ?>"><i class="fa fa-sign-out"></i> <?php _e('退出登录','moedog'); ?></a>
                </p>
            </div>
        </div>
