...
<div id="comments" class="comments-area">
    ...
    if(comments_open()){
        ...
        $args=array(
            ...
            'comment_field'=>'<div class="comment form-group has-feedback"><div class="input-group"><textarea class="form-control" id="comment" placeholder="'.__('|´・ω・)ノ看完说句话吧，什么都行！！','moedog').'" name="comment" rows="5" aria-required="true" required  onkeydown="if(event.ctrlKey){if(event.keyCode==13){document.getElementById(\'submit\').click();return false}};"></textarea></div><div class="OwO"></div></div>',
        );
        comment_form($args);
    }elseif(is_single()){
        echo '<div id="respond" class="comment-respond"><p style="color:#555;font-size:18px;margin-bottom:10px">'.__('当前页面评论已关闭。','moedog').'</p></div>';
    } ?>
</div>